...
export async function getDealsToday({ fallbackLimit = 6 } = {}) {
  const sql = getDb();
  if (!sql) return [];
  try {
    const today = await sql`
      ...
      ORDER BY discount_pct DESC NULLS LAST, detected_at DESC
      LIMIT 12
    `;
    ...
    console.error("[deals] getDealsToday:", e.message);
    return [];
  }
}
...
export function fmtDateTime(date, lang = "es") {
  return new Date(date).toLocaleString(lang === "en" ? "en-GB" : "es-ES", {
    day: "numeric",
    month: "short",
    year: "numeric",
    hour: "2-digit",
